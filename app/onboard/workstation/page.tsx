...
export default function WorkstationOnboardPage() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Best-effort ensure current user is attached to this workspace (in case server couldn't resolve session)
      try {
      ...
      // Refresh the session to get updated workspaceId
      await update();

      toast({
        title: "Onboarding complete",
        description: `Workspace created. Your role: ${data.role}`,
      });
      router.push("/admin");
    } catch (err: any) {
      toast({
    ...
    <div className="min-h-screen flex items-center justify-center p-6">
      <div className="w-full max-w-2xl bg-background border border-border rounded-xl p-6">
        ...
        <p className="text-muted-foreground mb-6">
          Please provide your workstation information. The first registered user
          in a workstation is assigned Admin by default.
        </p>
        <form
          onSubmit={onSubmit}
